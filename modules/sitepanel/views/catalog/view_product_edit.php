   ...
   <div class="content">
    ...
    <?php
       ...
       <td align="left">
          ...
          $i=1;
          $serv_array = array();
          $servprice_array = array();
          if(is_array($services) && !empty($services))
          {
            foreach($services as $val)
            {
                $serv_array[$val['service_id']] = $val['service_id'];
                $servprice_array[$val['service_id']] = $val['price'];
            }
          }
          $serv = $serv_array;
           foreach($service_type_arr as $key=>$val){
           $checked = isset($serv[$i])?"checked='checked'":"";
           $value = ($this->input->post("price"))?$this->input->post("price"):$servprice_array;
           ?>
           <input type="checkbox" name="service[]" class="serv" <?php echo $checked;?> value="<?php echo $key?>"><?php echo $val;?>&nbsp; &nbsp;<div class="servin" style="display:none;"><input type="text" class="spring_test" style="witdh:350px;" value="<?php echo isset($value[$i])?$value[$i]:'';?>" name="price[]"></div></div><br>
           <?php $i++;}?>
            </td>
      </tr>
		  ...
		  <table id="images" class="form">
		   ...
		   <tr class="trOdd">
             ...
             <tr class="trOdd">
			  ...
			  <td height="26" align="center" >:</td>
			  <td align="left">
              	<input name="logo_type" value="logo"<?php echo ($this->input->post('logo_type')=="logo") ? '  checked="checked"': ' checked="checked"';?> type="radio" disabled /> Company Logo
                &nbsp;
                <input name="logo_type" value="number"<?php echo ($this->input->post('logo_type')=="number") ? '  checked="checked"': '';?> type="radio" checked /> Company No.
              </td>
			 </tr>
