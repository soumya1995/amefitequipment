...
$type = (int) ($this->uri->segment(3)=='warranty')?"W":"P";
$title = ($type=='P')?"Package":"Warranty";
?>
<div id="content">
 ...
 <div class="box">
  ...
  <?php echo anchor_popup(base_url().'sitepanel/products/addservice/'.$type.'/'.$product_id.query_string(), 'Add '.$title, $atts_edit1);?>
     ...
     <tr>
      <td align="center" >Search [ Warranty/Package Name ]
       ...
       if( $this->input->get_post('keyword')!='' || $this->input->get_post('status')!='' || $this->input->get_post('type')!='' ){
		       ...
		       echo anchor("sitepanel/products/packages/".$product_id,'<span>Clear Search</span>');
  ...
  <div class="content">
   ...
   if( is_array($res) && !empty($res) )
   {
	   ...
	   $product_res=get_db_single_row('wl_products','product_name,product_code,products_id',$Condwherw=' AND products_id="'.$this->uri->segment(4).'"');
	   ...
	   <table width="100%">
	    <tr>
	     <td align="left"><strong>Product Name</strong> - <?php echo $product_res['product_name'];?><br/><strong>Product Code -</strong> <?php echo $product_res['product_code'];?><br/></td>
			 ...
			 foreach($res as $catKey=>$pageVal){
				...
				// $wres=$this->weight_model->get_weight_by_id($pageVal['weight_id']);
                 ...
                 <?php /*?> <td align="center" class="center"><?php echo $pageVal['quantity'];?></td><?php */?>
				  ...
				  <?php echo anchor_popup('sitepanel/products/editservice/'.$pageVal['type'].'/'.$pageVal['product_id'].'/'.$pageVal['attribute_id'], 'Edit', $atts_edit);?> 
                  <br />
				  <?php 				  
				   if($pageVal['default_status']!='1') {
				  		echo anchor('sitepanel/products/delete_price_variant/'.$pageVal['type'].'/'.$pageVal['product_id'].'/'.$pageVal['attribute_id'], 'Delete', $atts_edit);
				   }
				  ?></td>
