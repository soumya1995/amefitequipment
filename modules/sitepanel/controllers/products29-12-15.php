...
exit;*/
	...
	public  function services($page = NULL){
		 ...
		 $service_type              =   (int) $this->uri->segment(4,0);
		 $product_id              =   (int) $this->uri->segment(5,0);

		 $condtion = " ";
		 $where="product_id = '".$product_id."' AND service_type = '".$service_type."' ";
		...
		$data['page_links']     =  admin_pagination($base_url,$config['total_rows'],$config['limit'],$offset);
		$data['heading_title']  =  'Manage Service';
		$data['res']            =  $res_array;
	...
	public function editservice(){
		...
		$this->form_validation->set_rules('type_id','Warranty/Package',"trim|required|unique[wl_product_attributes.type_id='".$this->input->post('type_id')."' AND wl_product_attributes.product_id='".$product_id."' AND wl_product_attributes.service_type='".$service_type."' AND type='".$this->input->post('type')."' AND attribute_id!='".$attribute_id."' ]");		
	...
	public function addservice(){
		...
		$this->form_validation->set_rules('type','Type',"trim|required");
		$this->form_validation->set_rules('type_id','Warranty/Package',"trim|required|unique[wl_product_attributes.type_id='".$this->input->post('type_id')."' AND wl_product_attributes.product_id='".$product_id."' AND wl_product_attributes.service_type='".$service_type."' AND type='".$this->input->post('type')."' ]");		
		//$this->form_validation->set_rules('quantity','Stock',"trim|required|is_numeric|greater_than[0]");
	 ...
	 {	
		...
		if ($product_id && $service_type && $attribute_id) {
			...
			redirect('sitepanel/products/services/'.$service_type.'/'.$product_id, '');
		}		
	...
	function bulk_uploading(){
		...
		if ($this->form_validation->run() == TRUE) {
			$this->load->library('upload');
			$data = $this->upload->my_upload('bulk_data', 'products');
	...
	public function import_bulk_data_from_excel_sheet($path = '') {
		...
		if (is_array($worksheet) && count($worksheet) > 0) {
			...
			for ($i = 2; $i <= $total; $i++) {
				...
				$product_name = $worksheet[$i]["B"];
				$product_code = $worksheet[$i]["C"];				
				$products_description=@$worksheet[$i]["D"];
				...
				if ($product_found == 0 && $urlcount==0) {
					...
					$posted_data['product_added_date']=$this->config->item('config.date.time');
					$productId = $this->product_model->safe_insert('wl_products',$posted_data,FALSE);
