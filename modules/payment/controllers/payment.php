...
class Payment extends Public_Controller
{
	...
	public function __construct()
	{
		...
		parent::__construct();
		$this->load->helper(array('payment/paypal','payment/PayZippy','payment/DirectPost','cart/cart','file'));
	...
	public function index()
	{
		...
		if( $this->input->post()!='' )
		{
			...
			if($this->input->post('pay_method') == "directPost" ){	
				   
				    $working_order_id =  $this->session->userdata('working_order_id');
				    $order_res = $this->order_model->get_order_master($working_order_id);
				    DirectPostForm($order_res);				
			}
			exit;
	...
	public function order_success()
	{
		...
		if($query_res->num_rows()>0){
			...
			foreach($order_list as $key=>$val){
				$prd_qty=$val['quantity'];
				$this->db->set('product_stock',"product_stock-$prd_qty",FALSE);
				$this->db->where('products_id', $val['products_id']);				
				$this->db->update('wl_products');
			}
		}
	...
	{		
		//trace($_GET);		
		$ordId = md5($this->input->get("orderid"));	
		$payment_method = 'directPay';
		
		if($this->input->get("responsetext")=="SUCCESS"){
