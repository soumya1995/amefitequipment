...
class about_model extends MY_Model {
    ...
    public function getabout($opts = array()) {
        ...
        if ($keyword != '') {
            $opts['condition'].= " AND LOWER(about_name) like '%" . strtolower($keyword) . "%'";
        }
        ...
        $result = $this->findAll('wl_about as a', $fetch_config);
    ...
    public function get_about_by_id($id) {
        ...
        if ($id > 0) {
            $condtion = "status !='2' AND about_id=$id";
            $fetch_config = array(
                'condition' => $condtion,
                'debug' => FALSE,
                'return_type' => "array"
            );
            $result = $this->find('wl_about', $fetch_config);
            return $result;
        }
    }

    public function get_featured_about($parent_id, $limit) {
        ...
        $result = $this->findAll('wl_about', $fetch_config);
        return $result;
    }
