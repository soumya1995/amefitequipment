...
foreach($this->cart->contents() as $items){
	...
	$amount=(($items['price']+$items['options']['warranty_price']+$items['options']['package_price'])*$items['qty']);
	$sub_total+=$amount;
	$mt=($i=='1')?"":"mt5";	
	
	if(!empty($items['options']['service_type']) && !empty($items['options']['warranty_id'])){

		$woption["where"] = array('product_id'=>$items['pid'],'type_id'=>$items['options']['warranty_id'],'service_type'=>$items['options']['service_type'],'type'=>'W');
	...
	if(!empty($items['options']['service_type']) && !empty($items['options']['package_id'])){
		...
		$poption["where"] = array('product_id'=>$items['pid'],'type_id'=>$items['options']['package_id'],'service_type'=>$items['options']['service_type'],'type'=>'P');
