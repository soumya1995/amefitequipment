...
class Product_model extends MY_Model
 {
	...
	public function get_product_attributes($data,$limit='NULL',$start='NULL'){
		...
		if(!empty($product_id) && !empty($type))	{
			...
			$condtion = "product_id = '".$product_id."' AND type='".$type."' " ;
			$fetch_config = array(
									'condition'=>$condtion,
...
/*---------Color media-------------*/
	...
	public function get_review($limit='10',$offset='0',$param=array())
	{
        ...
        public function get_product_services($limit='4',$offset='0',$param=array())
	{
		$status			    =   @$param['status'];
		$productid			=   @$param['productid'];
		$orderby			=	@$param['orderby'];
		...
		if($status!='')
		{
			$this->db->where("ser.status","$status");
		}
		...
		$this->db->select('SQL_CALC_FOUND_ROWS ser.*,prd.product_name',FALSE);
		$this->db->from('tbl_product_services as ser');
		//$this->db->where('rev.status !=2');
		$this->db->join('wl_products AS prd','ser.product_id=prd.products_id','left');
		if(is_array($join)){
			$this->db->join($join["tableName"],$join["cond"],$join["type"]);
		}
		$q=$this->db->get();
